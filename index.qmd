---
title: 'Computational notebook supplement to "A Tutorial for Deception Detection Analysis or: How I Learned to Stop Aggregating Veracity Judgements and Embraced Bayesian Mixed Effects Models"'
author: 
  - name: Matti Vuorre
...
This document is a computational notebook supplement to "A Tutorial for Deception Detection Analysis or: How I Learned to Stop Aggregating Veracity Judgements and Embraced Bayesian Mixed Effects Models" ([Zloteanu & Vuorre, 2023](https://psyarxiv.com/fdh5b/)). We introduce Signal Detection Theory (SDT) and show how to apply it in practice in the area of deception research. Moreover, we implement SDT models as Generalized Linear Mixed Models (GLMM) in a bayesian inferential framework in R. This document is a [Quarto](https://quarto.org/) computational notebook. You can download the source code from [GitHub](https://github.com/mvuorre/deception-sdt) or [OSF](https://osf.io/kuhzj/) and reproduce the analyses in [RStudio](https://posit.co/downloads/).
...
However, this simple measure of accuracy can conflate any potential bias (to e.g. prefer "lie" over "not lie" responses) with their ability to discriminate between lies and true stimuli. Signal Detection Theory allows studying bias and sensitivity to lies separately.
...
p_m0_coef_participant <- m0 %>% 
  spread_draws(b_Intercept, b_isLie1, r_Participant[Participant,Parameter] | Parameter) %>% 
  mutate(
...
emm_m1 <- bind_rows(
  ...
  mutate(
    Parameter = str_remove(Parameter, "Yes - No, ") %>% 
      fct_inorder(),
    x = "dprime"
  ),
  ...
  mutate(
    t = if_else(str_detect(Parameter, " - "), "Differences", "Group means") %>% 
      fct_inorder(),
    across(where(is.numeric), ~if_else(x == "Criterion", -1 * ., .))
  )
...
x2 <- x %>% 
  group_by(name) %>%
  compare_levels(value, Training, comparison = emmeans_comparison("pairwise"))
tmp <- bind_rows(x, x2) %>% 
  mutate(
    t = if_else(str_detect(Training, "-"), "Differences", "Group means") %>% 
      fct_inorder(),
    Training = factor(Training, levels = levels(emm_m1$Parameter))
  )
```
